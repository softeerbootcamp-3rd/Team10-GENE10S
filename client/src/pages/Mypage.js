...
export default function Mypage() {
  ...
  useEffect(() => {
    ...
    axios.get('v1/reservation/list').then(response => setReservationList(response.data.data));
  }, []);
    ...
    <div className={classNames('page')}>
      ...
      <div className={classNames('title-area')}>
        ...
        <div className={classNames('title-fixed')}>
          <div className={classNames('title-text')}>My Page</div>
        </div>
      </div>
      ...
      <div className={classNames('body')}>
        <div className={classNames('menu-frame')}>
          ...
          {currentMenu ? (
            <ProfileCard profileInfo={profileInfo} carList={carList} />
          ) : (
            <ReservationCard reservationInfo={reservationInfo} />
          )}
        </div>
      </div>
      <Footer />
